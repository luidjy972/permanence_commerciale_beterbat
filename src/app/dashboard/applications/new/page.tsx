...
export default function NewApplicationPage() {
  ...
  // Determine current step: only advance when the USER has answered
  // messages[0] is the hidden auto-prompt, so real user answers start at index 2, 4, 6...
  const userAnswers = messages.filter((m, i) => m.role === 'user' && i > 0).length
  const step = Math.min(userAnswers, 5)
  const steps = [
    'Titre & Description',
    'Fonctionnalités',
    'Cas d\'utilisation',
            ...
            <div key={i} className="flex items-center gap-2">
              <div
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                  i < step
                    ...
                    : i === step
                    ? 'bg-red-500/10 text-red-500 ring-1 ring-red-400/30'
              ...
              >
                {i < step ? (
                  ...
                  <span className="w-3.5 h-3.5 rounded-full border flex items-center justify-center text-[9px]" style={{ borderColor: 'var(--color-border)' }}>
                    {i + 1}
                  </span>
                )}
